...
@router.get("/title/{title}", response_description="Get a single book by name", response_model=Book)
def find_book_by_name(title: str, request: Request):
    print(title)
    if (book := request.app.database["books"].find_one({"title": title})) is not None:
        return book

    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Book with ID {title} not found")
